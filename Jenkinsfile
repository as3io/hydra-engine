    ...
    try {
      ...
      docker.withRegistry('https://664537616798.dkr.ecr.us-east-1.amazonaws.com', 'ecr:us-east-1:aws-jenkins-login') {
        stage('Build Container') {
          myDocker = docker.build("hydra-engine:v${env.BUILD_NUMBER}", '.')
        }
      ...
      stage('Upgrade Container') {
        rancher confirm: true, credentialId: 'rancher', endpoint: 'https://rancher.as3.io/v2-beta', environmentId: '1a18', image: "664537616798.dkr.ecr.us-east-1.amazonaws.com/hydra-engine:v${env.BUILD_NUMBER}", service: 'hydra/engine', environments: '', ports: '', timeout: 180
      }
      ...
      stage('Notify NewRelic') {
        sh "curl -X POST 'https://api.newrelic.com/v2/applications/[NEED_NR_APP_ID]/deployments.json' \
